...
export async function fetchPageOfTicketsForUser(userId, token, page = 1, pageSize = 10) {
    const response = await fetch(`http://localhost:5000/users/${userId}/pageOfTickets?page=${page}&pageSize=${pageSize}`, {
        method: "GET",
        headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${token}`,
